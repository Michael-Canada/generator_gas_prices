...
# ISO_name = ["MISO", "SWPP"]  # Add more ISO names as needed
# ISO_name = "SWPP"
...
def get_hub_gas_prices():
    dt_strs = ["2021-", "2022-", "2023-", "2024-"]  # Add more year strings as needed
    # dt_strs = ["2023-"]  # Add more year strings as needed
    like_conditions = " OR ".join([f"filename LIKE '%{dt_str}%'" for dt_str in dt_strs])

    benchmark = duckdb.read_parquet(
        "/Users/michael.simantov/Documents/generator_gas_prices/*.parquet",
        filename=True,
    )
    hub_prices = duckdb.sql(f"SELECT * FROM benchmark WHERE {like_conditions}").to_df()
    hub_prices["month"] = hub_prices.filename.map(
        lambda f: int(f.split("/")[-1].split(".")[0].split("-")[1])
    )
    hub_prices["year"] = hub_prices.filename.map(
        lambda f: int(f.split("/")[-1].split(".")[0].split("-")[0])
    )
...
def get_hub_gas_prices_2023_only():
    dt_str_2023 = "2023-"
    # dt_str_2024 = "2024-"
    benchmark = duckdb.read_parquet(
        "/Users/michael.simantov/Documents/generator_gas_prices/*.parquet",
    ...
    hub_prices = duckdb.sql(
        f"SELECT * FROM benchmark WHERE filename LIKE '%{dt_str_2023}%'"
    ).to_df()
...
# the function reads data from an excel document: to be replaced with Vlad's development
def read_data(file):
...
def find_closest_hubs(
    ...
    def find_similarity_between_hub_and_generator_prices(
        ...
        fuel_prices = generator_fuel_data_per_month.merge(
            hub_gas_prices, on="year_month", how="inner"
        ).set_index("year_month")
        fuel_prices = fuel_prices.dropna()

        if fuel_prices.shape[0] < 3:
    ...
    for i in range(len(generators)):
            ...
            for j in range(len(NG_hub_loc_info)):
                ...
                generator_fuel_data_per_month = (
                    generator_consumption_and_price_df_summary[
                        ...
                        == generator.plant_id
                    ].sort_values(["year_month"])[["year_month", "fuel_cost"]]
                )  # ['fuel_cost'].values
                ...
                mean_hub_price = np.mean(hub_gas_prices.set_index("year_month").values)
        ...
        # if no gas hub was found within the distance range, continue to the next generator
        if len(distances) == 0:
            continue
...
def get_data_from_EIA_file(file):
    data = read_data(file)
    generator_consumption_and_price = []
    this_data = []
    for d in data:
        if d.fuel_group == "Natural Gas" and d.ba_code:
            this_data = [
                d.year,
                d.month,
                d.plant_id,
    ...
    generator_consumption_and_price_df.drop(
        columns=["average_heat_content"], inplace=True
    )

    # generators_no_fuel_cost = generator_consumption_and_price_df[generator_consumption_and_price_df.plant_id.isin(generator_consumption_and_price_df33)].groupby(['plant_id','year','balancing_authority_code']).agg({'plant_id':'first', 'plant_name':'first', 'year':'first', 'balancing_authority_code':'first','quantity':'sum'})
    # generators_no_fuel_cost = generators_no_fuel_cost.reset_index(drop=True)
    # generators_no_fuel_cost.to_csv('generators_no_fuel_cost.csv', index=False)

    generator_consumption_and_price_df = (
        generator_consumption_and_price_df.dropna()
    )  # ZZZ
    ...
    # merge the two dataframes
    generator_consumption_and_price_df_summary = (
        generator_consumption_and_price_df_summary.merge(
            generator_more_data, on="plant_id"
        )
    )

    return generator_consumption_and_price_df_summary
...
NG_hub_loc_info.to_csv(
    f"NG_hub_loc_info.csv", index=False
)  # This is not used anywhere. No need to save it as it is almost identical to the parquet file that was read a few lines above

hub_gas_prices_per_month = (
    hub_gas_prices.groupby(["month", "year", "price_symbol"])
    .agg({"mid_point": "mean"})
    .reset_index()
).sort_values(["year", "month"])

# create a new column named 'month_year', which is the concatenation of 'month' and 'year':
hub_gas_prices_per_month["year_month"] = (
    hub_gas_prices_per_month["year"].astype(str)
    + "-"
    # + hub_gas_prices_per_month["month"].astype(str)
    + hub_gas_prices_per_month["month"].astype(str).str.zfill(2)
)

hub_gas_prices_pivot = hub_gas_prices_per_month.pivot(
    index="year_month", columns="price_symbol", values="mid_point"
)
hub_gas_prices_pivot.sort_values("year_month", ascending=True)  # ZZZ
...
# list_of_EIA_file_names = ["EIA923_Schedules_2_3_4_5_M_12_2023_Early_Release.xlsx", "EIA923_Schedules_2_3_4_5_M_12_2024_Early_Release.xlsx"]
list_of_EIA_file_names = [
    "EIA923_Schedules_2_3_4_5_M_12_2023_Early_Release.xlsx",
    "EIA923_Schedules_2_3_4_5_M_12_2022_Final.xlsx",
    # "EIA923_Schedules_2_3_4_5_M_12_2021_Final_Revision.xlsx",
    "EIA923_Schedules_2_3_4_5_M_06_2024_20AUG2024.xlsx",
]

for ind, file_name in enumerate(list_of_EIA_file_names):
    generator_consumption_and_price_df_summary = get_data_from_EIA_file(file_name)

    generator_consumption_and_price_df_summary.to_csv(
        f"generator_consumption_and_price_df_summary_{ind}.csv", index=False
    )
# The following will be read in the file "example_multi_output_regressor_3.py"
hub_gas_prices_pivot.to_pickle("hub_gas_prices_pivot.pkl")

# Reading files that we have saved earlier:
generator_consumption_and_price_df_summary_0 = pd.read_csv(
    "generator_consumption_and_price_df_summary_0.csv"
)
generator_consumption_and_price_df_summary_1 = pd.read_csv(
    "generator_consumption_and_price_df_summary_1.csv"
)
generator_consumption_and_price_df_summary_2 = pd.read_csv(
    "generator_consumption_and_price_df_summary_2.csv"
)
# concatenate generator_consumption_and_price_df_summary_1 under generator_consumption_and_price_df_summary_0
generator_consumption_and_price_df_summary = pd.concat(
    [
        generator_consumption_and_price_df_summary_2,
        generator_consumption_and_price_df_summary_1,
        generator_consumption_and_price_df_summary_0,
    ]
).sort_values(["year", "month"])

# add a column named "year-month"
generator_consumption_and_price_df_summary["year_month"] = (
    generator_consumption_and_price_df_summary["year"].astype(str)
    + "-"
    + generator_consumption_and_price_df_summary["month"].astype(str).str.zfill(2)
)
# remove the columns "year" and "month":
generator_consumption_and_price_df_summary.drop(columns=["year", "month"], inplace=True)
generator_consumption_and_price_df_summary.to_csv(
    f"generator_consumption_and_price_df_summary.csv", index=False
)

# we stop here in case that we do not want to prepare data for the mao that is made in 'present_results.py'
# exit()
############### TESTING
if False:
    results = []
...
generators_with_close_hubs.dropna(inplace=True)
