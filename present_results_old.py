...
# find_similarity_between_hub_and_generator_prices(hub_gas_prices, generator_price_timeseries)
    ...
    plt.title(f'quantity: {row["quantity"]}, Matching_score: {np.round(100*row["current_supplier__Pvalue"], 0)}', fontsize=20)
...
<div style="position: fixed; 
    ...
    The percentage saving is shown in the popup when clicking on the red lines.<br><br>
    Click on a generator (blue dot) to see timeseries<br><br>
    Click on a red line to see the percentage saving associated with chosing a gas hub that is not the closest one<br>
</div>
'''
