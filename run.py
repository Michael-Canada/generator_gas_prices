...
def find_closest_hubs(generator_consumption_and_price_df_summary, NG_hub_loc_info, hub_gas_prices_pivot, CLOSEST_INTERESTING_GAS_HUB):
    ...
    for i in range(len(generators)):
        ...
        # if not generator['plant_id'] == 6089:
        #     continue
        ...
        for j in range(len(NG_hub_loc_info)):            
            ...
            if distance > CLOSEST_INTERESTING_GAS_HUB:
                continue
            ...
            MSE, mean_error, delta_prices = find_similarity_between_hub_and_generator_prices(hub_gas_prices, generator_fuel_data_per_month)
...
this_data = []
for d in data:
    if d.fuel_group == "Natural Gas" and d.ba_code == 'MISO':
        this_data = [d.year, d.month, d.plant_id, d. plant_name, d.quantity, d.fuel_cost, d.fuel_group, d.ba_code]
        generator_consumption_and_price.append(this_data)

generator_consumption_and_price_df = pd.DataFrame(generator_consumption_and_price, columns=["year", "month", "plant_id", "plant_name", "quantity", "fuel_cost", "fuel_group","balancing_authority_code"])

# only keep rows where there is no null
generator_consumption_and_price_df = generator_consumption_and_price_df.dropna()
...
plt.figure()
generators_with_close_hubs = find_closest_hubs(generator_consumption_and_price_df_summary, NG_hub_loc_info, hub_gas_prices_pivot, 1600)
...
# plt.figure()
# find 3 closest gas hubs to each generator
generators_with_close_hubs = find_closest_hubs(generator_consumption_and_price_df_summary, NG_hub_loc_info, hub_gas_prices_pivot, 1000)
